...
//
// ## 0.19.29 的重要更正（两轮零代码思考的结论，务必先读这两份）
//
// `doc/research/2026-09-26-column-sandbox-round1-thinking.md` 与
// `...-round2-thinking.md` 查清了一件推翻前提的事：
//
//   **并列三列走的是控制面通路**（`web-control.js` 的 `POST chat`），那条路
//   **只回文本、不执行工具**；而网页聊天页（chatglm.cn / kimi / …）**根本没有文件
//   工具**。所以原版这里写的「你的产出请写入 `<dir>`，不要直接修改主工作树」是一条
//   **做不到的指令** —— 模型最好的结果是无视它，最坏的结果是声称写了文件而磁盘上
//   什么都没有。那条措辞已在 0.19.29 改成**事实陈述**（产出由插件落盘、你没有文件工具）。
//
//   同时「沙箱」的落点也定清了：**本插件唯一能设围栏的地方是自己写文件的那一处**，
//   即新模块 `column-fs.js` —— 它按官方的 canonicalize-then-contain 口径，把每列产出
//   写进 `<workspace>/.hwb/cols/<scope>-<key>/`，越界抛 `COLUMN_FS_DENIED`。
//   它是 **containment，不是内核边界**，也**不拦模型**（模型侧没有写的能力，谈不上拦）。
//   真正的 OS 级隔离只有「每列一个独立 DSH 会话」能做到，那要改产品且需跨层改造，
//   故**明确不做**，只写进 round2 文档。
...
export function columnGuidance(ctx) {
  ...
  const body = ctx.role === 'review'
    ...
    : [
      ...
      // 0.19.29：这句话以前是「你的产出请写入 `<dir>`，不要直接修改主工作树」——
      // 那是一条**做不到的指令**：并列列走控制面通路（`web-control.js` 的 `POST chat`），
      // 那条路只回文本、不执行工具，而网页聊天模型**根本没有文件工具**。
      // 让模型去写一个它没有工具可写的目录，最好的结果是它无视，最坏的结果是它
      // 声称写了文件而磁盘上没有 —— 用户去找会发现目录是空的。
      // 现在改成陈述**事实**：产出由插件自动落盘（`column-fs.js` 的围栏写入口），
      // 模型只需把方案写进回答。约定因此从「谎话」变成「可执行」。
      `你的**回答正文**会被自动保存到 \`${dir}\`（本列专属目录），所以请把完整方案写在回答里。`,
      '你没有文件读写工具：**不要**声称自己创建或修改了任何文件；'
        + '需要引用别的列的结论时，请明确写出它来自哪一列。',
    ];
  const tail = '（说明：以上是并列会话之间的**工作区约定**；本列产出目录由插件写入并受路径围栏保护，'
    + '但这不是强制隔离——真正的文件权限仍由 Harness 的工具权限与审批决定。）';
