// live-view.test.mjs — 工作区画面流（路线 B）的护栏（0.20.0）。
//
// 钉住三层：
//   ① 输入映射纯函数（lib/live.js mapMouseInput / mapKeyInput / parseClientMessage）：
//      客户端消息 → CDP Input 参数的形状收敛。反向线：非法动作/缺坐标/超长消息
//      必须被丢弃或裸通过，绝不把垃圾形状送进 CDP。
//   ② hub 全行为（假驱动 + 假 CDP 会话 + 真 WebSocket）：建连握手（hello/pages）、
//      帧下发与 ack、输入派发、非回环 Origin 拒绝。CDP 会话只依赖 {send,on,off,
//      detach} 最小接口，因此可以在无浏览器的情况下把 hub 的每条消息路径都跑到。
//   ③ 接线结构：driver 必须带 live API 并放进返回对象；中继必须有 upgrade 钩子；
//      客户端必须有 LivePane 与镜像回落按钮。
import test from 'node:test';
import assert from 'node:assert/strict';
import http from 'node:http';
import { readFileSync } from 'node:fs';
import { fileURLToPath } from 'node:url';
import { dirname, join } from 'node:path';
import { mapMouseInput, mapKeyInput, parseClientMessage, createLiveHub, viewportForPanel, pickStreamMode, STREAM_MODES } from '../lib/live.js';

const here = dirname(fileURLToPath(import.meta.url));
const pkg = dirname(here);

// ---- ① 输入映射纯函数 -----------------------------------------------------

test('鼠标按下：完整形状（type/x/y/button/clickCount）', () => {
  const p = mapMouseInput({ action: 'pressed', x: 12.6, y: 8.2, button: 'left', buttons: 1, clickCount: 1 });
  assert.equal(p.type, 'mousePressed');
  assert.equal(p.x, 13);           // 取整，不做浮点坐标
  assert.equal(p.y, 8);
  assert.equal(p.button, 'left');
  assert.equal(p.buttons, 1);
  assert.equal(p.clickCount, 1);
});

test('滚轮：mouseWheel + delta 原样透传（缺省补 0）', () => {
  const p = mapMouseInput({ action: 'wheel', x: 1, y: 2, deltaY: -120 });
  assert.equal(p.type, 'mouseWheel');
  assert.equal(p.deltaY, -120);
  assert.equal(p.deltaX, 0);
});

test('反向线：未知动作 / 缺坐标 / 按钮越界 → null 或收敛', () => {
  assert.equal(mapMouseInput({ action: 'teleport', x: 1, y: 2 }), null);
  assert.equal(mapMouseInput({ action: 'pressed', y: 2 }), null);
  const p = mapMouseInput({ action: 'pressed', x: 1, y: 2, button: 'laser', buttons: 99 });
  assert.equal(p.button, 'none');
  assert.equal(p.buttons, 7);      // 位掩码钳到 0..7
});

test('键盘：可打印键带 text，keyup 无 text，修饰键位掩码', () => {
  const down = mapKeyInput({ action: 'key', key: 'a', code: 'KeyA', keyCode: 65, text: 'a', ctrl: true, shift: true });
  assert.equal(down.type, 'keyDown');
  assert.equal(down.text, 'a');
  assert.equal(down.windowsVirtualKeyCode, 65);
  assert.equal(down.modifiers, 2 | 8);   // ctrl=2, shift=8
  const up = mapKeyInput({ action: 'keyup', key: 'a', code: 'KeyA', keyCode: 65 });
  assert.equal(up.type, 'keyUp');
  assert.equal(up.text, undefined);
  assert.equal(up.modifiers, 0);
});

test('parseClientMessage：合法对象过，垃圾/超长/数组一律 null', () => {
  assert.deepEqual(parseClientMessage('{"t":"mouse"}'), { t: 'mouse' });
  assert.equal(parseClientMessage('not json'), null);
  assert.equal(parseClientMessage('[1,2]'), null);
  assert.equal(parseClientMessage('"' + 'x'.repeat(5000) + '"'), null);
  assert.equal(parseClientMessage(''), null);
});

// ---- ② hub 全行为（假驱动 + 真 WebSocket）---------------------------------

/** 假 CDP 会话：记录 send 调用，可手动触发 on 注册的事件。 */
function fakeCdp(pageId) {
  const sent = [];
  const handlers = new Map();
  let detached = false;
  return {
    sent,
    pageId,
    send: async (cmd, params) => { sent.push([cmd, params]); return {}; },
    on: (ev, cb) => { handlers.set(ev, cb); },
    off: (ev) => { handlers.delete(ev); },
    detach: async () => { detached = true; },
    get isDetached() { return detached; },
    emit: (ev, payload) => handlers.get(ev)?.(payload),
  };
}

function fakeDriver() {
  const cdp = fakeCdp('p1');
  const driver = {
    live: {
      async listPages() { return [{ id: 'p1', title: 'Test Page', url: 'about:blank', active: true }]; },
      async attach() { return cdp; },
      async openPage() { return []; },
      async closePage() { return { ok: true }; },
      async activatePage() { return { ok: true }; },
      onPagesChanged() { return () => {}; },
    },
  };
  return { driver, cdp };
}

/** 起一个临时 http server + hub，返回 {url, close, hub}。 */
async function startHub(getDriver) {
  const hub = createLiveHub({ getDriver, log: () => {}, warn: () => {} });
  const server = http.createServer(() => {});
  await new Promise((res) => server.listen(0, '127.0.0.1', res));
  server.on('upgrade', (req, socket, head) => hub.handleUpgrade(req, socket, head));
  const port = server.address().port;
  return { hub, port, close: () => new Promise((res) => server.close(res)) };
}

test('hub 全行为：握手 → 页面列表 → 二进制帧+ack → 鼠标派发 → 关闭清场', async () => {
  const { driver, cdp } = fakeDriver();
  const { port, close } = await startHub(() => driver);
  const ws = new WebSocket(`ws://127.0.0.1:${port}/webcode/live?account=deepseek`);
  ws.binaryType = 'arraybuffer';
  const received = [];
  const frames = [];
  ws.onmessage = (ev) => {
    if (typeof ev.data === 'string') { received.push(JSON.parse(ev.data)); return; }
    // 与客户端同一套二进制解码：[metaLen u16be][metaJSON][jpeg]
    const view = new DataView(ev.data);
    const metaLen = view.getUint16(0);
    const meta = JSON.parse(new TextDecoder().decode(new Uint8Array(ev.data, 2, metaLen)));
    frames.push({ meta, jpeg: new Uint8Array(ev.data, 2 + metaLen) });
  };
  await new Promise((res, rej) => { ws.onopen = res; ws.onerror = rej; });

  // 握手 + 页面列表（hub 建连即推，且默认附到驱动当前页 → startScreencast 已发）
  await new Promise((r) => setTimeout(r, 150));
  assert.equal(received[0].t, 'hello');
  assert.equal(received[0].account, 'deepseek');
  const pagesMsg = received.find((m) => m.t === 'pages');
  assert.equal(pagesMsg.pages[0].id, 'p1');
  assert.ok(cdp.sent.some(([cmd]) => cmd === 'Page.startScreencast'), '建连必须启动投屏');

  // 模拟一帧损伤：二进制下发 + ack（jpeg 字节必须与 base64 解码后逐字一致）
  cdp.emit('Page.screencastFrame', { data: 'ZkBSQU1F', metadata: { deviceWidth: 1280, deviceHeight: 800 }, sessionId: 7 });
  await new Promise((r) => setTimeout(r, 100));
  assert.equal(frames.length, 1, '帧必须以二进制包到达');
  assert.equal(frames[0].meta.deviceWidth, 1280);
  assert.equal(Buffer.from(frames[0].jpeg).toString('utf8'), 'f@RAME', 'jpeg 字节逐字保真（无 base64 二次转换）');
  const ack = cdp.sent.find(([cmd, p]) => cmd === 'Page.screencastFrameAck');
  assert.deepEqual(ack[1], { sessionId: 7 }, '每帧必须 ack，否则 Chromium 停发');

  // 输入派发：合法鼠标消息 → CDP 参数；非法消息 → 丢弃不抛
  ws.send(JSON.stringify({ t: 'mouse', action: 'pressed', x: 10, y: 20, button: 'left', buttons: 1 }));
  await new Promise((r) => setTimeout(r, 100));
  const dispatch = [...cdp.sent].reverse().find(([cmd]) => cmd === 'Input.dispatchMouseEvent');
  assert.equal(dispatch[1].type, 'mousePressed');
  assert.equal(dispatch[1].x, 10);
  ws.send(JSON.stringify({ t: 'mouse', action: 'teleport' }));
  ws.send('garbage');
  await new Promise((r) => setTimeout(r, 80));

  ws.close();
  await new Promise((r) => setTimeout(r, 100));
  assert.ok(cdp.isDetached, '连接关闭必须 detach CDP 会话');
  await close();
});

test('安全面：非回环 Origin 的 upgrade 被拒绝（socket 立即销毁）', async () => {
  const { driver } = fakeDriver();
  const { port, close } = await startHub(() => driver);
  const err = await new Promise((resolve) => {
    const req = http.request({
      host: '127.0.0.1', port, path: '/webcode/live?account=deepseek',
      headers: { connection: 'Upgrade', upgrade: 'websocket', origin: 'https://evil.example',
        'sec-websocket-key': 'x3JJHMbDL1EzLkh9GBhXDw==', 'sec-websocket-version': 13 },
    });
    req.on('upgrade', () => resolve(null));
    req.on('error', resolve);
    req.on('close', () => setTimeout(() => resolve(new Error('socket closed')), 30));
    req.end();
  });
  assert.ok(err, '非回环 Origin 不得完成 upgrade');
  await close();
});

test('安全面：驱动缺 live API（如旧版驱动/未知槽抛错）→ 回 bye 并关闭', async () => {
  const { port, close } = await startHub(() => ({}));
  const ws = new WebSocket(`ws://127.0.0.1:${port}/webcode/live?account=deepseek`);
  const first = await new Promise((res) => { ws.onmessage = (ev) => res(JSON.parse(ev.data)); });
  assert.equal(first.t, 'bye');
  await close();
});

// ---- 视口自适应（0.20.2 ×2 超采样 → 0.21.1 保比例 → 0.21.2 对齐画布）-------
//
// 三代判据，别混：
//   0.20.2  「宽高双钳制（720–1280 / 900–2000）」——把失配钉成规格。
//   0.21.1  「比例严格等于面板」——修掉黑边，**但 1280×2000 上限没动**。
//   0.21.2  「视口 = 面板 CSS × dpr = 画布 backing」——修掉**上采样糊**。
//
// 0.21.1 修完仍糊的原因：客户端画布 backing = 面板CSS × dpr，而服务端把页面
// 渲染成「面板CSS × k，k ≤ min(2, 1280/宽, 2000/高)」。只要 面板宽 × dpr > 1280
// （临界点 640 CSS px），源图就小于画布 ⇒ drawImage 上采样。

const VP_MAX_W = 2560, VP_MAX_H = 3200;

test('viewportForPanel：视口 = 面板 × dpr = 画布 backing（0.21.2 的核心不变量）', () => {
  // ① 未触上限时**逐字等于** 面板×dpr。这是「源图 = 画布、不做任何重采样」的
  //    充分必要条件；任何一端改口径都会让这条失败。
  for (const [w, h] of [[460, 860], [400, 400], [300, 900], [500, 1000], [640, 900], [1280, 1600]]) {
    assert.deepEqual(viewportForPanel(w, h), { width: w * 2, height: h * 2 },
      `${w}×${h} @dpr2 必须等于画布 backing ${w * 2}×${h * 2}`);
  }
  // ② 典型侧栏：数值与 0.20.2/0.21.1 逐字相同（回归面为零）
  assert.deepEqual(viewportForPanel(460, 860), { width: 920, height: 1720 });
  assert.deepEqual(viewportForPanel(500, 1000), { width: 1000, height: 2000 });
  // ③ 缺省/非法回落固定值（此时无从对齐）
  assert.deepEqual(viewportForPanel(), { width: 1024, height: 1440 });
  assert.deepEqual(viewportForPanel(-5, 0), { width: 1024, height: 1440 });
  assert.deepEqual(viewportForPanel(0, 0), { width: 1024, height: 1440 });
});

test('viewportForPanel：dpr 跟面板走，且与 client 的 panelDpr() 同口径（上限 2）', () => {
  // dpr=1（普通屏）：视口就是面板 CSS 尺寸，不放大
  assert.deepEqual(viewportForPanel(1000, 800, 1), { width: 1000, height: 800 });
  assert.deepEqual(viewportForPanel(1000, 800, 1.25), { width: 1250, height: 1000 });
  assert.deepEqual(viewportForPanel(1000, 800, 1.5), { width: 1500, height: 1200 });
  assert.deepEqual(viewportForPanel(1000, 800, 2), { width: 2000, height: 1600 });
  // dpr 超过上限：与 client.cjs 的 panelDpr() 一样钳到 2（两端必须同口径）
  assert.deepEqual(viewportForPanel(1000, 800, 3), { width: 2000, height: 1600 });
  // dpr 缺失/非法：回落上限 2（等同旧行为）
  assert.deepEqual(viewportForPanel(1000, 800, undefined), { width: 2000, height: 1600 });
  assert.deepEqual(viewportForPanel(1000, 800, 0), { width: 2000, height: 1600 });
  assert.deepEqual(viewportForPanel(1000, 800, -1), { width: 2000, height: 1600 });
});

test('viewportForPanel：超上限时按比例收缩，比例误差仍 < 0.5%', () => {
  // 上限只是防面板撑爆编码，触顶后**仍要保持比例**（不能再回到独立钳制）
  const panels = [[300, 900], [400, 400], [460, 860], [500, 1000], [600, 300],
    [900, 300], [1200, 700], [1400, 900], [2400, 1200], [3840, 1080]];
  for (const [w, h] of panels) {
    const vp = viewportForPanel(w, h);
    const panelAspect = w / h, vpAspect = vp.width / vp.height;
    const err = Math.abs(vpAspect / panelAspect - 1);
    assert.ok(err < 0.005,
      `${w}×${h} 比例失配 ${(err * 100).toFixed(2)}%（面板 ${panelAspect.toFixed(3)} vs 视口 ${vpAspect.toFixed(3)}）`);
    assert.ok(vp.width <= VP_MAX_W && vp.height <= VP_MAX_H,
      `${w}×${h} 突破上限: ${vp.width}×${vp.height}`);
  }
  // 全屏工作区（旧实现最坏 −68% 比例 / 2.19× 上采样）现在两样都正常
  const full = viewportForPanel(2400, 1200);
  assert.deepEqual(full, { width: 2560, height: 1280 });
  assert.equal(full.width / full.height, 2400 / 1200, '超上限时比例仍须严格保持');
});

test('viewportForPanel：上限已抬高到覆盖 1280×1600 面板 @dpr2（0.21.2 修糊的前提）', () => {
  // 旧上限 1280×2000 会让这一档（官方右侧栏全屏常见尺寸）上采样 2 倍
  assert.deepEqual(viewportForPanel(1280, 1600), { width: 2560, height: 3200 });
  // 且它确实等于画布 backing（这条就是「不糊」的判据本身）
  const vp = viewportForPanel(1280, 1600);
  assert.equal(vp.width, 1280 * 2);
  assert.equal(vp.height, 1600 * 2);
});

test('hub 行为：建连即下发视口仿真；resize 变尺寸同会话重设（不重建会话）', async () => {
  const { driver, cdp } = fakeDriver();
  const { port, close } = await startHub(() => driver);
  const ws = new WebSocket(`ws://127.0.0.1:${port}/webcode/live?account=deepseek`);
  const received = [];
  ws.onmessage = (ev) => received.push(JSON.parse(ev.data));
  await new Promise((res, rej) => { ws.onopen = res; ws.onerror = rej; });
  await new Promise((r) => setTimeout(r, 150));
  assert.ok(cdp.sent.some(([cmd, p]) => cmd === 'Emulation.setDeviceMetricsOverride' && p.width === 1024 && p.height === 1440),
    '建连默认视口必须有 Emulation 仿真（超采样分辨率）');
  const startBefore = cdp.sent.filter(([cmd]) => cmd === 'Page.startScreencast').length;

  ws.send(JSON.stringify({ t: 'resize', w: 460, h: 860 }));
  await new Promise((r) => setTimeout(r, 120));
  const emu = [...cdp.sent].reverse().find(([cmd]) => cmd === 'Emulation.setDeviceMetricsOverride');
  assert.deepEqual([emu[1].width, emu[1].height], [920, 1720], 'resize 必须按面板 ×2 重设视口');
  assert.equal(cdp.sent.filter(([cmd]) => cmd === 'Page.startScreencast').length, startBefore + 1,
    'resize 重启投屏但不重建 CDP 会话');
  // 重启必须先 stop 再 start（否则新上限不保证生效）
  const lastStop = cdp.sent.map(([cmd]) => cmd).lastIndexOf('Page.stopScreencast');
  const lastStart = cdp.sent.map(([cmd]) => cmd).lastIndexOf('Page.startScreencast');
  assert.ok(lastStop > -1 && lastStop < lastStart, '投屏重启顺序必须 stop → start');

  // 同尺寸重复 resize：跳过（拖拽连发的防抖兜底）
  const before = cdp.sent.length;
  ws.send(JSON.stringify({ t: 'resize', w: 460, h: 860 }));
  await new Promise((r) => setTimeout(r, 100));
  assert.equal(cdp.sent.length, before, '同尺寸 resize 不得重发任何 CDP 命令');

  ws.close();
  await new Promise((r) => setTimeout(r, 100));
  await close();
});

test('三档画质：首帧前 PNG → 密集帧 JPEG q55 半分辨率 → 静止回 PNG 全分辨率', async () => {
  const { driver, cdp } = fakeDriver();
  const { port, close } = await startHub(() => driver);
  const ws = new WebSocket(`ws://127.0.0.1:${port}/webcode/live?account=deepseek`);
  ws.binaryType = 'arraybuffer';
  ws.onmessage = () => {};
  await new Promise((res, rej) => { ws.onopen = res; ws.onerror = rej; });
  await new Promise((r) => setTimeout(r, 150));
  const lastCast = () => [...cdp.sent].reverse().find(([cmd]) => cmd === 'Page.startScreencast')?.[1];

  // 首帧前 = idle：无损 PNG，且是**全分辨率**（这时最该清晰，成本又最低）
  assert.equal(lastCast().format, 'png', '首帧前必须是 PNG（无损）');
  assert.equal(lastCast().maxWidth, 1024, 'idle 档必须是全分辨率，不下采样');

  // 密集帧（间隔 ~50ms < motionGapMs 90）⇒ motion：JPEG q55 且半分辨率
  for (let i = 0; i < 6; i++) {
    cdp.emit('Page.screencastFrame', { data: 'ZkBSQU1F', metadata: {}, sessionId: 10 + i });
    await new Promise((r) => setTimeout(r, 45));
  }
  await new Promise((r) => setTimeout(r, 400));
  assert.equal(lastCast().format, 'jpeg', '运动态用 JPEG（PNG 编码太贵）');
  assert.equal(lastCast().quality, 55, '运动态必须降到 q55 提帧率');
  assert.ok(lastCast().maxWidth < 1024, '运动态必须下采样出帧（省编码），' + lastCast().maxWidth);

  // 静下来 ⇒ idle：回 PNG 全分辨率（消灭 JPEG 的 8×8 DCT 伪影）
  await new Promise((r) => setTimeout(r, 1400));
  assert.equal(lastCast().format, 'png', '静止后必须回 PNG 无损（文字零伪影）');
  assert.equal(lastCast().maxWidth, 1024, '静止后必须回全分辨率');

  ws.close();
  await new Promise((r) => setTimeout(r, 100));
  await close();
});

test('pickStreamMode：纯函数判据全边界（离线可驱动）', () => {
  assert.equal(pickStreamMode({ sinceLastMs: Infinity }), 'idle', '首帧前 = idle');
  assert.equal(pickStreamMode({ sinceLastMs: 900 }), 'idle', '久无帧 = idle');
  assert.equal(pickStreamMode({ sinceLastMs: 50, gapEwma: 200 }), 'stream', '中速 = stream');
  assert.equal(pickStreamMode({ sinceLastMs: 50, gapEwma: 40 }), 'motion', '快速 = motion');
  assert.equal(pickStreamMode({ sinceLastMs: 50, gapEwma: 0 }), 'stream', 'EWMA 未建立 = stream');
  assert.equal(pickStreamMode({}), 'idle', '空参 = idle');
  assert.equal(pickStreamMode(), 'idle', '无参 = idle');
});

test('STREAM_MODES：idle 无损全分辨率、motion 半分辨率（高精度 + 低占用的落点）', () => {
  assert.equal(STREAM_MODES.idle.format, 'png');
  assert.equal(STREAM_MODES.idle.scale, 1);
  assert.equal(STREAM_MODES.motion.format, 'jpeg');
  assert.ok(STREAM_MODES.motion.scale < 1, 'motion 必须下采样');
  assert.ok(STREAM_MODES.motion.quality < STREAM_MODES.stream.quality, 'motion 质量必须低于 stream');
});

test('WebRTC：rtc-offer 注入目标页、answer 回传、成功停投屏；rtc-failed 回落投屏', async () => {
  const { driver, cdp } = fakeDriver();
  // 假 CDP 对 Runtime.evaluate 回一个成功 answer
  cdp.send = async (cmd, params) => {
    cdp.sent.push([cmd, params]);
    if (cmd === 'Runtime.evaluate') {
      assert.ok(params.expression.includes('getDisplayMedia'), '注入脚本必须走页面自采');
      assert.ok(params.awaitPromise === true, '注入必须 await');
      return { result: { value: JSON.stringify({ ok: true, sdp: 'ANSWER-SDP', candidates: [{ candidate: 'c1' }] }) } };
    }
    return {};
  };
  const { port, close } = await startHub(() => driver);
  const ws = new WebSocket(`ws://127.0.0.1:${port}/webcode/live?account=deepseek`);
  ws.binaryType = 'arraybuffer';
  const msgs = [];
  ws.onmessage = (ev) => { if (typeof ev.data === 'string') msgs.push(JSON.parse(ev.data)); };
  await new Promise((res, rej) => { ws.onopen = res; ws.onerror = rej; });
  await new Promise((r) => setTimeout(r, 150));
  ws.send(JSON.stringify({ t: 'rtc-offer', sdp: 'OFFER-SDP', candidates: [] }));
  await new Promise((r) => setTimeout(r, 150));
  const answer = msgs.find((m) => m.t === 'rtc-answer');
  assert.ok(answer, '必须回 rtc-answer');
  assert.equal(answer.sdp, 'ANSWER-SDP');
  assert.ok(cdp.sent.some(([cmd]) => cmd === 'Page.stopScreencast'), 'RTC 接管后必须停投屏');
  ws.send(JSON.stringify({ t: 'rtc-failed' }));
  await new Promise((r) => setTimeout(r, 120));
  const lastStart = [...cdp.sent].reverse().find(([cmd, p]) => cmd === 'Page.startScreencast');
  assert.ok(lastStart, 'rtc-failed 后必须重启投屏（自动降级）');
  ws.close();
  await new Promise((r) => setTimeout(r, 100));
  await close();
});

// ---- RTC 状态机守卫（0.21.1）：rtcActive 必须挡住所有投屏重启路径 ----------
//
// 0.21.0 的 rtcActive 只被**写**（rtc-offer / rtc-failed / close），没有被任何产帧
// 路径读——于是两条路径都能在 RTC 接管后把 JPEG 投屏**重新拉起**，而客户端此时
// canvas 是 visibility:hidden，编码出来的帧没人看：
//   · adaptTimer（250ms 轮询）判定要切画质 → startStream
//   · resize 分支（拖分栏每 120ms 一次）看到视口变了 → startStream

/** 把 CDP 的 Runtime.evaluate 变成「RTC 一定成功」的假实现。 */
function rtcOkDriver() {
  const { driver, cdp } = fakeDriver();
  cdp.send = async (cmd, params) => {
    cdp.sent.push([cmd, params]);
    if (cmd === 'Runtime.evaluate') {
      return { result: { value: JSON.stringify({ ok: true, sdp: 'ANSWER-SDP', candidates: [] }) } };
    }
    return {};
  };
  return { driver, cdp };
}

const startsOf = (cdp) => cdp.sent.filter(([cmd]) => cmd === 'Page.startScreencast').length;

async function enterRtc(ws, msgs) {
  ws.send(JSON.stringify({ t: 'rtc-offer', sdp: 'OFFER-SDP', candidates: [] }));
  await new Promise((r) => setTimeout(r, 200));
  assert.ok(msgs.some((m) => m.t === 'rtc-answer'), '前置条件：必须已进入 RTC 模式');
}

test('RTC 守卫 A：rtcActive 时 resize 不得重启投屏（拖分栏不再双路烧 CPU）', async () => {
  const { driver, cdp } = rtcOkDriver();
  const { port, close } = await startHub(() => driver);
  const ws = new WebSocket(`ws://127.0.0.1:${port}/webcode/live?account=deepseek`);
  ws.binaryType = 'arraybuffer';
  const msgs = [];
  ws.onmessage = (ev) => { if (typeof ev.data === 'string') msgs.push(JSON.parse(ev.data)); };
  await new Promise((res, rej) => { ws.onopen = res; ws.onerror = rej; });
  await new Promise((r) => setTimeout(r, 150));
  await enterRtc(ws, msgs);

  const afterRtc = startsOf(cdp);
  // 拖分栏：连发多次不同尺寸（客户端 120ms 防抖下真机就是这个节奏）
  for (const [w, h] of [[520, 900], [700, 950], [900, 1000]]) {
    ws.send(JSON.stringify({ t: 'resize', w, h }));
    await new Promise((r) => setTimeout(r, 60));
  }
  await new Promise((r) => setTimeout(r, 200));
  assert.equal(startsOf(cdp), afterRtc,
    'RTC 模式下 resize 绝不能重启 JPEG 投屏（画面已由 getDisplayMedia 接管）');
  // 视口数值仍须更新（RTC 失败回落时要用最新尺寸）
  const emu = [...cdp.sent].reverse().find(([cmd]) => cmd === 'Emulation.setDeviceMetricsOverride');
  assert.ok(emu, 'RTC 模式下 resize 仍要记录视口（回落时用）');

  ws.close();
  await new Promise((r) => setTimeout(r, 100));
  await close();
});

test('RTC 守卫 B：rtcActive 时自适应定时器不得重启投屏', async () => {
  const { driver, cdp } = rtcOkDriver();
  const { port, close } = await startHub(() => driver);
  const ws = new WebSocket(`ws://127.0.0.1:${port}/webcode/live?account=deepseek`);
  ws.binaryType = 'arraybuffer';
  const msgs = [];
  ws.onmessage = (ev) => { if (typeof ev.data === 'string') msgs.push(JSON.parse(ev.data)); };
  await new Promise((res, rej) => { ws.onopen = res; ws.onerror = rej; });
  await new Promise((r) => setTimeout(r, 150));
  await enterRtc(ws, msgs);

  const afterRtc = startsOf(cdp);
  // 制造「运动态」帧节奏：ADAPTIVE 判定 500ms 内 ≥3 帧就会想切 lo 画质。
  // 若缺 rtcActive 守卫，250ms 轮询 + dwellMs 700ms 后必然 startStream。
  for (let i = 0; i < 12; i++) {
    cdp.emit('Page.screencastFrame', { data: 'ZkBSQU1F', metadata: {}, sessionId: 100 + i });
    await new Promise((r) => setTimeout(r, 50));
  }
  // 覆盖完整的 250ms 轮询 + 700ms dwell 窗口
  await new Promise((r) => setTimeout(r, 1600));
  assert.equal(startsOf(cdp), afterRtc,
    'RTC 模式下自适应定时器绝不能重启 JPEG 投屏（视频已接管，帧没人看）');

  ws.close();
  await new Promise((r) => setTimeout(r, 100));
  await close();
});

test('RTC 守卫反向线：rtc-failed 之后 resize 仍须正常重启投屏（守卫不能太宽）', async () => {
  const { driver, cdp } = rtcOkDriver();
  const { port, close } = await startHub(() => driver);
  const ws = new WebSocket(`ws://127.0.0.1:${port}/webcode/live?account=deepseek`);
  ws.binaryType = 'arraybuffer';
  const msgs = [];
  ws.onmessage = (ev) => { if (typeof ev.data === 'string') msgs.push(JSON.parse(ev.data)); };
  await new Promise((res, rej) => { ws.onopen = res; ws.onerror = rej; });
  await new Promise((r) => setTimeout(r, 150));
  await enterRtc(ws, msgs);
  // 降级回投屏
  ws.send(JSON.stringify({ t: 'rtc-failed' }));
  await new Promise((r) => setTimeout(r, 150));

  const before = startsOf(cdp);
  ws.send(JSON.stringify({ t: 'resize', w: 520, h: 900 }));
  await new Promise((r) => setTimeout(r, 200));
  assert.equal(startsOf(cdp), before + 1,
    '回落投屏后 resize 必须照常重启投屏——守卫只该挡 RTC 活跃期');

  ws.close();
  await new Promise((r) => setTimeout(r, 100));
  await close();
});

// ---- ③ 接线结构（无浏览器无法实例化 driver/client 的部分按仓库惯例源码断言）---

const driverSrc = readFileSync(join(pkg, 'lib', 'browser-driver.js'), 'utf8');
const indexSrc = readFileSync(join(pkg, 'lib', 'index.js'), 'utf8');
const clientSrc = readFileSync(join(pkg, 'lib', 'client.cjs'), 'utf8');
const liveSrcOf = () => readFileSync(join(pkg, 'lib', 'live.js'), 'utf8');

test('接线：liveHeaded 有头三件套（旗标+隐藏）与客户端 video/降级结构', () => {
  assert.ok(driverSrc.includes('cfg.liveHeaded === true ? [') && driverSrc.includes('--use-fake-ui-for-media-stream'), '有头分支必须有 RTC 相关旗标');
  assert.ok(driverSrc.includes('function hideWindowFromTaskbar(') && driverSrc.includes('WS_EX_TOOLWINDOW'.slice(0,0) + 'SetWindowLongPtr'), '必须有任务栏隐藏 helper');
  assert.ok(indexSrc.includes('liveHeaded: true,') && (indexSrc.match(/liveHeaded: cfg.liveHeaded/g)||[]).length >= 2, 'liveHeaded 必须声明并双槽传入');
  assert.ok(clientSrc.includes('new RTCPeerConnection()') && clientSrc.includes('rtc-failed'), '客户端必须有 WebRTC 与自动降级');
  assert.ok(clientSrc.includes('hwb-live-video'), '必须有 video 元素');
});

test('接线：driver 返回对象带 live；activatePage 绝不改写自动化页；拒绝关自动化页', () => {
  assert.ok(/setImageLimitsProvider, live \}/.test(driverSrc), 'driver 返回对象必须含 live');
  const start = driverSrc.indexOf('const live = {');
  const end = driverSrc.indexOf('sessionSlot 与 status', start);
  const body = driverSrc.slice(start, end);
  assert.ok(body.includes('bringToFront'), 'activatePage 用 bringToFront（不 repoint 自动化页）');
  assert.ok(body.includes("reason: 'automation-page'"), '关闭自动化页必须拒绝');
  assert.ok(body.includes('newCDPSession'), 'attach 走 ctx.newCDPSession');
});

test('接线：中继挂 onUpgrade；index.js 建 liveHub 并接 driverFor', () => {
  const relaySrc = readFileSync(join(pkg, 'lib', 'relay.js'), 'utf8');
  assert.ok(relaySrc.includes("httpServer.on('upgrade', cfg.onUpgrade)"));
  // 0.21.2：createLiveHub 的入参从单行展开成多行（多了 onAllClosed 回收钩子）
  assert.ok(indexSrc.includes('createLiveHub({'));
  assert.ok(indexSrc.includes('getDriver: (accountKey) => driverFor(accountKey)'));
  assert.ok(indexSrc.includes('onAllClosed:'), '必须有右栏全关后的浏览器回收钩子');
});

test('接线：右栏全关回收浏览器——busy 期间不关（不能掐断正在生成的回复）', () => {
  const start = indexSrc.indexOf('onAllClosed:');
  assert.ok(start > -1, 'index.js 必须定义 onAllClosed');
  const body = indexSrc.slice(start, start + 1600);
  assert.ok(body.includes("st?.busy === true"), 'busy 判定必须在回收前');
  assert.ok(body.includes('return;'), 'busy 时必须直接返回、不关浏览器');
  assert.ok(body.includes('d.close'), '非 busy 时才关');
  // 驱动仍留在 drivers 表里 ⇒ 下一轮 ensure() 能重新拉起，回收是可逆的
  assert.ok(!body.includes('drivers.delete'), '回收不得把驱动从表里摘掉（否则下一轮拿不到它）');
});

test('接线：客户端有 LivePane、LIVE_SITES（先 DeepSeek）与镜像回落按钮', () => {
  assert.ok(clientSrc.includes('function LivePane('));
  assert.ok(clientSrc.includes('f.live') && clientSrc.includes('Object.entries(frames).map(([sid, f]) => f.live'), '画面流必须是显式 opt-in（默认镜像，0.21.2 路线还原）');
  assert.ok(clientSrc.includes('真实模式'), '镜像路线必须有进入真实模式的入口');
  assert.ok(clientSrc.includes('/webcode/live?account='));
});

test('接线：客户端上报 resize（含 dpr）且 canvas 按同一 dpr 绘制', () => {
  // dpr 必须一并上报：服务端据此把页面渲染成「面板 CSS × dpr」= 画布 backing，
  // 两端算出同一个数才是 1:1（源图 = 画布，无上采样）。
  assert.ok(clientSrc.includes("send({ t: 'resize', w: Math.round(box.clientWidth), h: Math.round(box.clientHeight), dpr: panelDpr() })"),
    '建连时的 resize 必须带 dpr');
  assert.ok(clientSrc.includes("send({ t: 'resize', w, h, dpr: panelDpr() })"),
    'ResizeObserver 的 resize 也必须带 dpr（拖到不同缩放比显示器时 dpr 会变）');
  assert.ok(clientSrc.includes('new ResizeObserver'), '容器尺寸变化必须上报');
  assert.ok(clientSrc.includes("Math.round(box.clientWidth * dpr)"), 'canvas 必须按 devicePixelRatio 放大');
  // 两端 dpr 口径必须一致（上限 2）——这是本次修复最容易回归的一点
  assert.ok(clientSrc.includes('const panelDpr = () => Math.max(1, Math.min(2, window.devicePixelRatio || 1))'),
    'client 的 panelDpr 必须钳到 [1,2]');
  assert.ok(liveSrcOf().includes('const VP_MAX_DPR = 2;'), 'live.js 的 VP_MAX_DPR 必须与 client 同口径');
});

test('接线：主题同步——客户端观察宿主昼夜属性并上报，hub 用 prefers-color-scheme 仿真', () => {
  assert.ok(clientSrc.includes("document.body.hasAttribute('data-ds-dark-theme')"),
    '判据必须取自 DSH 自己的深色标记属性');
  assert.ok(clientSrc.includes("send({ t: 'theme', dark:"), '客户端必须上报昼夜');
  assert.ok(clientSrc.includes('new MutationObserver'), '昼夜切换必须被观察到（建连即报 + 变化时报）');
  assert.ok(liveSrcOf().includes("case 'theme'"), 'hub 必须处理 theme 消息');
  assert.ok(liveSrcOf().includes('Emulation.setEmulatedMedia'), '必须用 prefers-color-scheme 仿真推给页面');
  assert.ok(liveSrcOf().includes("name: 'prefers-color-scheme'"), '仿真的媒体特性名必须正确');
});

test('接线：面板内自建状态行已删（用户要求「多的这一行删除」）', () => {
  // 上面 .hwb-toolbar 已含站点名+状态；面板内那条 bar 是重复信息
  assert.ok(!clientSrc.includes("className: 'hwb-live-bar'"), '面板内状态行必须已删');
  assert.ok(!clientSrc.includes("className: 'hwb-live-status'"), '状态文字节点必须已删');
});

test('接线：「+」开的页必须立刻成为画面（用户「你直接新开了页面干嘛不用」）', () => {
  // openPage 返回新页 id，hub 收到后立刻 attach 过去
  assert.ok(driverSrc.includes('return { ok: true, pageId: livePageId(p) };'), 'openPage 必须返回新页 id');
  const liveSrc = liveSrcOf();
  const openCase = liveSrc.slice(liveSrc.indexOf("case 'open'"), liveSrc.indexOf("case 'close'"));
  assert.ok(openCase.includes('res?.pageId'), 'hub 必须读新页 id');
  assert.ok(openCase.includes('await attach(res.pageId)'), 'hub 必须立刻附到新页');
});

test('反向线：点击换算必须对齐 dpr 坐标空间（0.20.2 真机「点击没反应」的首因）', () => {
  assert.ok(clientSrc.includes('(e.clientX - r.left) * f.dpr'), '鼠标 CSS 像素必须乘 dpr 对齐 backing-store 空间');
});
