...
export function createWebControl(deps = {}) {
  ...
  const actions = {
    'GET status': async () => ({
      ok: true,
      relay: relay ? { running: relay.status().running, consent: relay.status().consent, busy: relay.status().busy, lastError: relay.status().lastError } : null,
      driver: driver ? driver.status() : null,
    }),
  ...
  async function handlePreview(req, res) {
    ...
    try {
      const b64 = await driver.screenshotBase64();
      if (!b64) return sendJson(req, res, { ok: false, error: 'driver not running' }, 503);
