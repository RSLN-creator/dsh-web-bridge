...
4. **发布边界（2026-09-26 发布车道收口后更新）**：本修复随 **0.19.24** 打包（其后因许可识别
   修正重打包为 **0.19.25**）装入双 profile，并与 DSH STORE 收录契约的 manifest / LICENSE
   修正同一车；代码已在默认分支。但对 **正在运行**的 `dsh web` 进程（PID 11268）仍需一次
   重启才生效——重启会终止在跑会话，由用户按自己的节奏决定，发布车道不代为重启。
