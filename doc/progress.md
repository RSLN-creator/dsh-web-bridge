...
| 单测基线 | **75/75 测试文件（886 条断言）通过**（2026-09-22 实跑 `node --test test/*.test.mjs`，exit 0）。台账曾写 70/70（0.16.38 读数）、72/72（0.16.40 初稿）、74/74（0.18.0 读数，当时 873 条断言）——三次数值都随新护栏文件过期，`check-ledger` 每次都把它判红（这正是该闸门存在的意义）。**本机沙箱下 `pnpm test` 起子进程会 `spawn EPERM`**（见 §0.16.34 五），本机复核一律用 `node --test test/*.test.mjs` 这一条。 |
