...
| 单测基线 | **84/84 测试文件全绿**（2026-09-24 逐文件实跑；84 个里 `reply-log` 需 `NODE_TEST_CONTEXT=1`，未设时该文件按设计判红一次，设后 4/4 通过）。本行上一次写的是 **83/83**（2026-09-23 逐文件实跑，本机沙箱下 `node --test test/*.test.mjs` 会 `spawn EPERM`，因此一律逐文件执行并设 `NODE_TEST_CONTEXT=1`）。台账曾写 81/81（0.19.3 读数）、80/80、75/75（当时 886 条断言）、74/74、72/72、70/70——数值随新护栏文件过期，`check-ledger` 每次都把它判红（这正是该闸门存在的意义）。本轮新增 3 个护栏文件（`token-density` / `relay-lanes`）并把 `client-render` / `control-routes` / `settings-transport` 扩到新语义；**断言总数本轮未汇总**。<br>**复核方式必须写清（本轮踩过一次）**：全量必须**串行独占**——本轮曾让两个作业并发跑测试，结果 `empty-response` 挂住 53 分钟、`regression` 从 8 分钟涨到 19 分钟；单独复跑 `empty-response` **69.8s / 5/5 通过**。并发下的红/慢**不算读数**（本文件 0.19.0 行已记过同型判据）。 |
