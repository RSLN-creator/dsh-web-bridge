...
| 上游 | **0.19.24 本轮**：在 `3de2089`（0.19.16–0.19.23 源码收口）之上提交本车并推送 `origin/main`（含 DSH STORE 收录契约 + GLM 无头调用残片修复）；推送后以远端为准。<br>**上一版行（保留）**：`origin/main` = `fb7cd6e`（0.16.31 文档收口）；本轮 0.16.32–0.16.40 待提交/待推（0.16.38 / 0.16.39 / 0.16.40 **均已打包装机**） |
