...
| 0.16.33 范围（历史） | 右栏 Web Bridge 二级站点菜单（官方 `Menu` 原语的 `submenu`）+ 账户行状态点；设置页**站点 tab 条**（形态对齐 dsh-market）+ 站点级排队间隔（`sendGapMsBySlot`，后端既有档位）；账户头像改用站点矢量标记 `SiteGlyph` |
| 已装版本（profile） | web = **0.19.12**、headless = **0.19.12**（2026-09-25 用 `dsh plugin --profile <p> add` 持久装入——**声明与内容一起动**，不走只写 `node_modules` 的易回退装法；两 profile 的 `package.json` 依赖、`pnpm-lock.yaml` specifier、`node_modules` 实装版本三方实测均为 0.19.12；tarball sha512 `rtWoSFyv/qlvgbL09R1e4Yr/PsO2CoBRebz01sYi2q6E6dgl8MyoPXvkDcAwLU5lC2pfNHgx1R83CRY1DbT8fQ==`；`lib/metrics.js` 新符号 `shouldRescueStalledCapture`/`cleanAnswerDomText` 与 `lib/browser-driver.js` 的 `CAPTURE_STALL_MS`/`dom-rescue-capture-stall` **全部就位**） |
| 运行中的进程 | 3080 侧 relay 实测 `/__webcode/status` 的 `build` = `{hash:'ad70f8c267de', version:'0.19.11'}`、`browserSource='bundled'`（chromium-1232）（2026-09-25 01:05 实读）⇒ 磁盘已是 **0.19.12**，**进程仍跑 0.19.11，待用户手动重启** |
