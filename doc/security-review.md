...
## 7. CodeQL 告警的逐条处置（2026-09-26 补）

一次 CodeQL 全量扫描在本包内产出 **202 条告警**。逐条看下来，**只有 2 条是真缺陷**，
其余约 200 条是「模式匹配命中」而不是「运行时可到达的洞」。本节记下这个判断与它的依据——
只报「已扫描」而不说「扫出来的一堆里哪两条算数、为什么其余不算数」，评审者无法复核。

### 7.1 两条真缺陷（已修）

| # | 严重度 | 位置 | 事实 | 处置 |
|---|---|---|---|---|
| 1 | critical | `lib/upstream.js`（重定向前后） | 重定向跟随**没有跨信任边界收窄**：从公网地址起步的请求，可以被 302 带到内网/回环地址，把内网响应取回来 | 新增 `SSRF_REDIRECT_BLOCKED`：`startedPrivate` 在 `attempt()` **之外**按最外层 URL 算一次；只有「起点是公网」时才拦后续私网跳转。回环起点（测试替身、本地镜像）行为不变 ⇒ **只收窄，不放宽**。护栏 `test/ssrf-redirect-guard.test.mjs` 4 项（含「起点公网 + 跳内网 ⇒ 必须抛错且内网零命中」与「起点回环 ⇒ 旧行为保持」） |
| 2 | high | `test-mock/prompt-bench.mjs` | 把失败原因拼进 markdown 表格时**没有转义反斜杠**（只转义了 `|`），含 `\` 的输入会把表格结构撑破 | 先转 `\` 再转 `\|`（顺序不可颠倒，反过来会把新加的转义符再转一次） |

两条都跑过**反向变异**：把修复行改回原样 ⇒ 对应用例必须变红；还原 ⇒ 全绿且哈希逐字不变。

### 7.2 为什么其余约 200 条不算缺陷

| 类别 | 条数（约） | 判据 |
|---|---|---|
| 测试 / 探针 / fixture 里的字符串 | 多数 | 命中的是 `test/`、`test-mock/`、`.tmp*/` 下的**测试数据**，不是随包发布的运行代码（`package.json` 的 `files` 白名单只有 `lib` / `bin` / `cordis.patch.yml` / `README.md` / `LICENSE`） |
| 防御性正则本身被当成「不安全的正则」 | 若干 | 例如私网地址识别、协议边界探测——这些正则的**用途就是**处理不可信输入，命中模式不等于存在缺陷 |
| 固定的本地回环 URL | 若干 | `127.0.0.1` 字面量出现在**桥自己**的控制面/镜像路径上，不是「由用户输入决定的请求目标」 |
| 已在上文 6.3 / 6.4 记为「未修项」的老面 | 若干 | 不与本节重复计数（本机进程可访问中继、profile 存真实 cookie 等），它们仍在未修清单里，**没有**因为这次扫描被降级 |

**本节不声称「已清零」。** 判据是「运行时可到达 + 跨信任边界」，不满足这条的告警留在 CodeQL 里
是**正确的**——把测试文件里的字符串改成让扫描器满意，只会让下次扫描失去信噪比。
